...
*
*/
export function num2form2(num: number, $before: string, $after: string): string {
  const $cases = [2, 0, 1, 1, 1, 2];
  return (
    $before[num % 100 > 4 && num % 100 < 20 ? 2 : $cases[Math.min(num % 10, 5)]] +
    ' ' +
    String(num) +
    ' ' +
...
*/
export function num2form(num: number, forma1: string, forma2: string, forma3: string): string {
  let forma;
